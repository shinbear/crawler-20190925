...
public class QBOne_0722 {
	...
	public static void main(String[] args) throws IOException {
		try {
			...
			String toptitle = "IDS\tNo\tID\tName\tname_en_last\tname_en_first"
					...
					+ "U1\tU2\tPU-PA\tSN\tEI\tJ9\tJI\tPD\tPY\tVL\tIS\tBP\tEP\tDI\tWC\tUT\tGA";
			...
			for (int i = startRow; i <= endRow; i++) {
				...
				for (int k = 0; k < 3; k++) {
					...
					if (source_Search.length() == 0 || source_Search == null) {
						continue;
					}
					source_SearchYearFrom = source_SearchYearFromArry[k];
					source_SearchYearTo = source_SearchYearToArry[k];
					String[] source_Search_arry = { source_Search, source_SearchYearFrom, source_SearchYearTo };
					...
					// Search keywords arrary
					recordlist.add(source_Search_arry);

					int runflag = 0;
					do {
						// Run the record
						recordlist = Runwebdriver_QBOne.runRecord(recordlist);
	...
	public static void writrintExcel() throws IOException {
		// write into excel
		writer.println(IDS + "\t" + No + "\t" + ID + "\t" + Name + "\t" + name_en_last + "\t" + name_en_first + "\t"
				+ aff_inst_candidate_year + "\t" + CAS_candidate_yr + "\t" + source_Search + "\t"
